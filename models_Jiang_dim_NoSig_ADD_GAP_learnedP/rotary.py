...
def apply_rot_emb(q, rot_emb):
    sin, cos = rot_emb
    rot_dim = sin.shape[-1]
    (q, q_pass) = map(lambda t: (t[..., :rot_dim], t[..., rot_dim:]), q)
    q = map(lambda t: t * cos + rotate_every_two(t) * sin, q)
    q = map(lambda t: torch.cat(t, dim=-1), (q, q_pass))
    return q
